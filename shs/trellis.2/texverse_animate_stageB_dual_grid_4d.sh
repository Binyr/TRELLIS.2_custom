#!/bin/bash
# Voxelize TexVerse-Animation Stage B renders into per-view dual-grid .vxz tars.
...
#     (rendered_v1_stageB/logs/progress_*.json) at startup via --render_logs_prefix,
#     so we don't need to pre-build a render_finished_view.json.
#
# Usage: bash shs/trellis.2/texverse_animate_stageB_dual_grid_4d.sh [world_size] [rank]
...
RENDER_LOGS_PREFIX="s3://arcwm-code-us-west-2/yanruibin/efs/4D_video_data_process/data/texverse_1k_animate/rendered_v1_stageB/logs/"
S3_INPUT="s3://arcwm-code-us-west-2/yanruibin/efs/4D_video_data_process/data/texverse_1k_animate/rendered_v1_stageB"
...
echo "================================================================"
echo "[voxel:texverseB] host=$(hostname) rank=$RANK/$WS resolution=$RESOLUTION"
