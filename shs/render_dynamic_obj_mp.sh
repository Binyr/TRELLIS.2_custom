#!/bin/bash
# Multi-process launcher for dynamic OBJ rendering.
#
# Per pod:
...
LOCAL_LOG_ROOT="/local-ssd/render_dynamic_obj_logs"
...
worker_supervisor() {
    ...
    while (( attempts < MAX_RESPAWN )); do
        ...
        echo "[supervisor:r${rank}] attempt #$attempts gpu=$gpu tag=$worker_tag pid_parent=$$" \
            | tee -a "$death_log"
        CUDA_VISIBLE_DEVICES=$gpu python tools/bl_rendering/batch_render_dynamic_obj.py \
            --manifest "$MANIFEST" \
