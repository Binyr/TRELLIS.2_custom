...
for i in $(seq 0 $((num_chunk-1))); do
    ...
    -c "cd /data/work/run_codes && ls -lh && source uv/setup.sh --new-env --venv-dir /local-ssd/vpixal3d-venv && export HF_TOKEN=\$HF_TOKEN && shs/download_github.sh $num_chunk $i" \
    -j "download-github-$i"
done
